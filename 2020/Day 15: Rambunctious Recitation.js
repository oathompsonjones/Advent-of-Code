...
for (let i = input.length; nums.length < 2020; ++i) {
    ...
    if (nums.slice(0, nums.length - 1).includes(lastNum)) {
        const indexesOfLastNum = nums.map((x, j) => ({ j, x })).filter((x) => x.x === lastNum)
            .map((x) => x.i);
        if (indexesOfLastNum.length === 1) {
            [nums[i]] = indexesOfLastNum;
        } else {
            nums[i] = indexesOfLastNum[indexesOfLastNum.length - 1] -
                indexesOfLastNum[indexesOfLastNum.length - 2];
        }
    } else {
        nums[i] = 0;
    }
