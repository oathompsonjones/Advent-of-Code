...
class Monkey {
    constructor(monkeyInput) {
        ...
        this.id = parseInt(lines.shift().trim()
            .split(" ")[1], 10);
        this.startingItems = lines.shift().trim()
            .split(": ")[1].split(", ").map(Number);
        [, this.operationText] = lines.shift().trim()
            .split(": new = old ");
...
const runRounds = (totalRounds, div) => {
