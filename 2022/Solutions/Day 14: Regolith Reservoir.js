...
const pathCoords = input.split("\n").map((path) => path.split(" -> ").map((coord) => ({ x: Number(coord.split(",")[0]), y: Number(coord.split(",")[1]) })));
...
const generatePathFromCoords = (coordArray) => {
    const coords = [];
...
const cellHasRock = ({ x, y }) => y === maxY || paths.flat().some(({ x: pathX, y: pathY }) => pathX === x && pathY === y);
const sandCoords = [];
const cellHasSand = ({ x, y }) => sandCoords.some(({ x: pathX, y: pathY }) => pathX === x && pathY === y);
const cellIsOccupied = ({ x, y }) => cellHasRock({ x, y }) || cellHasSand({ x, y });
const drawGrid = () => {
    const xValues = Array(maxX - minX + 1).fill(0)
        .map((_, i) => minX + i);
    let grid = `    ${xValues.map((value) => Math.floor(value / 100 % 10)).join(" ")}\n    ${xValues.map((value) => Math.floor(value / 10 % 10)).join(" ")}\n    ${xValues.map((value) => Math.floor(value / 1 % 10)).join(" ")}\n`;
