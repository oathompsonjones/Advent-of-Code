...
// Part 1
const timestamp = input.shift() as number;
const busTimes: Record<PropertyKey, number[]> = {};

input.filter((x): x is number => x !== "x").forEach((x) => {
    busTimes[x] = [];
...
const earliest = Math.min(...Object.keys(busTimes).map((x) => busTimes[x]!)
    .flat());
const busID = parseInt(Object.keys(busTimes).find((x) => busTimes[x]!.includes(earliest))!, 10);
